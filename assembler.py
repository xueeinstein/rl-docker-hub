# Modified from github: tensorflow/tensorflow/tools/dockerfiles/assembler.py
"""Multipurpose RL Simulators Docker Helper.
...
flags.DEFINE_string('hub_password', None, (
...
flags.DEFINE_string(
    'hub_repository', None,
    'Push tags to this Docker Hub repository, e.g. xueeinstein/rl-hub')
...
flags.DEFINE_boolean('construct_dockerfiles',
                     False,
                     'Do not build images',
                     short_name='d')

flags.DEFINE_boolean('keep_temp_dockerfiles',
                     False,
                     'Retain .temp.Dockerfiles created while building images.',
                     short_name='k')

flags.DEFINE_boolean('build_images',
                     False,
                     'Do not build images',
                     short_name='b')

flags.DEFINE_string('run_tests_path', None, (
  ...
  """
    ...
    """
        if isfullarg and '=' not in value:
            self._error(field,
                        '{} should be of the form ARG=VALUE.'.format(value))
        if not isfullarg and '=' in value:
            self._error(field,
                        '{} should be of the form ARG (no =).'.format(value))
...
def build_name_from_slices(format_string, slices, args, is_dockerfile=False):
    ...
    name_formatter.update({
        ...
        for s in slices if is_dockerfile and 'dockerfile_exclusive_name' in s
    })
...
def get_slice_sets_and_required_args(slice_sets, tag_spec):
  ...
  """
    ...
    possible_args_or_slice_set_names = extract_bracketed_words.findall(
        tag_spec)
  ...
  """
    ...
    for name, release in spec['releases'].items():
        for tag_spec in release['tag_specs']:
            ...
            slice_combos = aggregate_all_slice_combinations(
                spec, used_slice_sets)
            for slices in slice_combos:
                ...
                dockerfile_contents = merge_partials(spec['header'],
                                                     used_partials,
                                                     all_partials)
  ...
  """
    ...
    for path, _, files in os.walk(partial_path):
        for name in files:
            ...
            simple_name = fullpath[len(partial_path) +
                                   1:-len('.partial.dockerfile')]
...
def main(argv):
    ...
    if FLAGS.upload_to_hub:
        if not FLAGS.hub_username:
            eprint(
                '> Error: please set --hub_username when uploading to Dockerhub.'
            )
            exit(1)
        ...
        if not FLAGS.hub_password:
            eprint(
                '> Error: please set --hub_password when uploading to Dockerhub.'
            )
            exit(1)
        dock.login(
    ...
    for tag, tag_defs in all_tags.items():
        for tag_def in tag_defs:
            ...
            if FLAGS.only_tags_matching and not re.match(
                    FLAGS.only_tags_matching, tag):
                eprint(
            ...
            if (is_x86 and any([arch in tag for arch in ['ppc64le']])
                    or not is_x86 and proc_arch not in tag):
                continue
            ...
            # directory)
            dockerfile = os.path.join(FLAGS.dockerfile_dir,
                                      tag + '.temp.Dockerfile')
            if not FLAGS.dry_run:
            ...
            if not FLAGS.dry_run:
                try:
                    ...
                    resp = dock.api.build(timeout=FLAGS.hub_timeout,
                                          path='.',
                    ...
                    while True:
                        try:
                            ...
                            eprint('Error parsing from docker image build: {}'.
                                   format(output))
                    ...
                    if FLAGS.run_tests_path:
                        ...
                        for test in tag_def['tests']:
                            ...
                            if code != 0:
                                eprint('>> {} failed tests with status: "{}"'.
                                       format(repo_tag, code))
                                failed_tags.append(tag)
                                tag_failed = True
                                if FLAGS.stop_on_failure:
                                    eprint(
                                        '>> ABORTING due to --stop_on_failure!'
                                    )
                                    exit(1)
                            else:
            ...
            if FLAGS.upload_to_hub:
                ...
                eprint('>> Uploading to {}:{}'.format(FLAGS.hub_repository,
                                                      tag))
                if not FLAGS.dry_run:
                    p = multiprocessing.Process(target=upload_in_background,
                                                args=(FLAGS.hub_repository,
                                                      dock, image, tag))
                    p.start()
